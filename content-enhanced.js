...
class GrammarGuardV2 {
  ...
  async init() {
    ...
    console.log(`GrammarGuard v2.0 initialized ✓ (Language: ${this.settings.language || 'en'})`);
  }
  ...
  analyzeText(text) {
    ...
    // Detect language
    const userLanguage = this.settings.language || 'en';
    const detectedLanguage = typeof LanguageDetector !== 'undefined' 
      ? LanguageDetector.getLanguageForSettings(text, userLanguage)
      : userLanguage;
    
    const rules = this.getEnhancedRules(detectedLanguage);
  ...
  getEnhancedRules(language = 'en') {
    let rules = [];
    
    // Add English rules if applicable
    if (language === 'en' || language === 'both') {
      rules = rules.concat(this.getEnglishRules());
    }
    
    // Add Filipino rules if applicable
    if (language === 'fil' || language === 'both') {
      rules = rules.concat(this.getFilipinoRules());
    }
    
    // If no specific language, default to English
    if (rules.length === 0) {
      rules = this.getEnglishRules();
    }
  ...
  getEnglishRules() {
    // Enhanced English rules with severity and explanations
    return [
      {
        pattern: /\bi\s/gi,
        replacement: 'I ',
      ...
      },
      {
        ...
        message: 'Use "There is"',
        category: 'grammar',
        severity: 'critical',
        explanation: '"Their" shows possession. Use "there is" to indicate existence or location.'
      },
      {
        pattern: /\byour\s+welcome\b/gi,
        replacement: "You're welcome",
        message: 'Use "You\'re welcome"',
        category: 'grammar',
        severity: 'critical',
        explanation: '"You\'re" (you are) is correct here, not "your" (possessive).'
      },
      {
        pattern: /\bits\s+a\s+nice\s+day\b/gi,
        replacement: "It's a nice day",
        message: 'Use "It\'s" (contraction)',
        category: 'grammar',
        severity: 'critical',
      ...
      },
      {
        ...
        replacement: 'receive',
        message: 'Correct spelling: "receive"',
        category: 'spelling',
        severity: 'critical',
      ...
      },
      {
        ...
        message: 'Correct spelling: "occurred"',
        category: 'spelling',
        severity: 'critical',
        explanation: 'Double the "r" in "occurred".'
      },
      {
        pattern: /\baccidently\b/gi,
        replacement: 'accidentally',
        message: 'Correct spelling: "accidentally"',
        category: 'spelling',
        severity: 'critical',
        explanation: '"Accidentally" ends with "-ally".'
      },
      {
        ...
        category: 'spelling',
        severity: 'critical',
        explanation: '"Definitely" has an "i" in the middle, not "a".'
      },
      {
        ...
        category: 'spelling',
        severity: 'critical',
        explanation: '"Separate" has an "a" in the middle.'
      },
      {
        pattern: /\s{2,}/g,
        replacement: ' ',
      ...
      },
      {
        ...
        severity: 'minor',
        explanation: 'One punctuation mark is sufficient.'
      },
      {
        pattern: /\bthru\b/gi,
        replacement: 'through',
        message: 'Use "through" in formal writing',
        category: 'style',
        severity: 'minor',
        explanation: '"Through" is more formal than "thru".'
      },
      {
        pattern: /\bu\b/gi,
        test: (match, fullText) => {
          // Don't flag if it's a single letter U (like in abbreviations)
          return fullText.split(/\s+/).length > 3;
        },
        replacement: 'you',
        message: 'Use "you" instead of "u"',
        category: 'style',
        severity: 'minor',
        explanation: 'Avoid text speak in formal writing.'
      },
      {
        pattern: /\bur\b/gi,
        replacement: 'your',
        message: 'Use "your" instead of "ur"',
        category: 'style',
        severity: 'minor',
        explanation: 'Avoid text speak in formal writing.'
      }
